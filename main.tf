...
module "app-ansible-exec-schematics" {
  ...
  depends_on	= [ module.app-vsi, local_file.ansible_inventory, local_file.db_ansible_saphana-vars, local_file.app_ansible_bw4app-vars, local_file.tf_ansible_hana_storage_generated_file ]
  count = (var.PRIVATE_SSH_KEY == "n.a" && var.BASTION_FLOATING_IP == "localhost" ? 0 : 1)
